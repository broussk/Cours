...
<html lang='fr'>
  ...
  <body>
    ...
    <form method="POST"action="http://localhost:8080/ajout" name="formulaire">
        <?php
        $recupModele = json_decode(file_get_contents('../src/modele.json'),true);
        $recupMoteur = json_decode(file_get_contents('../src/moteur.json'),true);
        $recupCouleur = json_decode(file_get_contents('../src/couleurs.json'),true);
        $recupOptions = json_decode(file_get_contents('../src/options.json'),true);
        ...
        ?>
        <br>
        <label>Numero Identification : </label><input type="text" name="numeroIdentification" placeholder="AA-BBB-CC (exemple : AB-540-DJ)" style="width: 220px;" required/>
        <br/>
        <br>
        ...
        <br/>
        <br>
        <label>Date de Fabrication : </label><input type="text" name="dateFabrication" placeholder="AAAA-JJ-MM (exemple : 2020-31-10)" style="width: 230px;"  required/>
        <br/>
        <br>
        ...
        <INPUT type="checkbox" name="options5" value="aucune options"> aucune options
        <br/>
        <br>
