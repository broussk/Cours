...
class TheSurvivorWindows extends JFrame
{
    ...
    protected JMenu jm_Affiche=new JMenu("Afficher");
	...
	jmi_Coucou.addActionListener(new ActionListener() 
	    {
		public void actionPerformed(ActionEvent ev)
		{
	    ...
	    for(i=0;i<this.island.length;i++)
	        ...
	        for(j=0;j<this.island[0].length;j++)
	            ...
	            etat = this.manager.getPosition(i, j);
	                if(etat == 2){
	                    island[i][j].setBackground(Color.BLUE);
	                    island[i][j].setText("R");
	                }
	                else{
	                    if(etat == 1){
	                    island[i][j].setText("");
	                    island[i][j].setBackground(Color.RED);
	                    }
	                    else{
	                    if(etat==3){
